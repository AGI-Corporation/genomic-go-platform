...
class SafetyMonitoringSystem:
    ...
    async def process_adverse_event(self, event: Dict):
        ...
        Optimized with O(1) amortized complexity using a sliding window.
        """
        ...
        while self.ae_buffer:
            try:
                first_event_dt = datetime.fromisoformat(self.ae_buffer[0]["timestamp"])
                if first_event_dt.tzinfo is None:
                    first_event_dt = first_event_dt.replace(tzinfo=timezone.utc)
            except (ValueError, KeyError):
                # If timestamp is invalid, remove it to prevent blocking
                popped = self.ae_buffer.popleft()
                if popped.get("severity", 0) >= 3:
                    self.severe_count -= 1
                continue

            if first_event_dt < cutoff_time:
                popped = self.ae_buffer.popleft()
                if popped.get("severity", 0) >= 3:
                    self.severe_count -= 1
            else:
                # Events are assumed to be roughly chronological
                break
        ...
        if len(self.ae_buffer) > 10:  # Minimum for statistical power
            ...
            if severe_ae_rate > self.safety_threshold:
                # Convert deque to list for alert processing
