...
class SafetyMonitoringSystem:
    ...
    def __init__(self, trial_id: str, safety_threshold: float = 0.15):
        ...
        # Bolt Optimization: Use deque for O(1) pruning from the left
        # Stores (event_time, event_dict) tuples
        self.ae_buffer = deque()
        # Bolt Optimization: Keep a running count of severe events to avoid O(N) summation
        self.severe_count = 0
        self.alert_history = []
    ...
    async def process_adverse_event(self, event: Dict):
        ...
        # Bolt Optimization: Parse timestamp once and store with event
        try:
            event_time = datetime.fromisoformat(event["timestamp"])
            # Ensure naive for comparison if needed (following existing pattern)
            if event_time.tzinfo is not None:
                event_time = event_time.replace(tzinfo=None)
        except (KeyError, ValueError):
            logger.error(f"Invalid timestamp in event: {event}")
            return
        ...
        # Bolt Optimization: O(1) sliding window pruning
        ...
        # Calculate rolling AE rate using pre-calculated severe_count and deque length
        ...
        if buffer_len > 10:  # Minimum for statistical power
            ...
            if severe_ae_rate > self.safety_threshold:
                # Compatibility: convert deque of tuples back to list of dicts for trigger_safety_alert
