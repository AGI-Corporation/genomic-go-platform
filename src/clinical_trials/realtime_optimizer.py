...
class SafetyMonitoringSystem:
    ...
    def __init__(self, trial_id: str, safety_threshold: float = 0.15):
        ...
        self.severe_count = 0  # Running counter for severe events (severity >= 3)
        self.alert_history = []
    ...
    async def process_adverse_event(self, event: Dict):
        """
        Process single adverse event and check for safety signals.
        Uses a sliding window (7 days) for O(1) amortized performance.
        """
        # Parse event timestamp (ensure it's UTC aware)
        try:
            event_time = datetime.fromisoformat(event["timestamp"])
            if event_time.tzinfo is None:
                event_time = event_time.replace(tzinfo=timezone.utc)
        except (KeyError, ValueError) as e:
            logger.error(f"Invalid event timestamp in trial {self.trial_id}: {e}")
            return

        # Add new event and its parsed timestamp to buffer
        self.ae_buffer.append((event_time, event))
        if event.get("severity", 0) >= 3:
            self.severe_count += 1

        # Prune expired events (older than 7 days) from the front of the deque
        cutoff = datetime.now(timezone.utc) - timedelta(days=7)
        while self.ae_buffer:
            oldest_time, oldest_event = self.ae_buffer[0]

            if oldest_time < cutoff:
                self.ae_buffer.popleft()
                if oldest_event.get("severity", 0) >= 3:
                    self.severe_count -= 1
            else:
                # Early exit: assume events arrive mostly in chronological order
                break

        # Calculate safety signal using pre-aggregated severe_count
        buffer_size = len(self.ae_buffer)
        if buffer_size > 10:
            severe_ae_rate = self.severe_count / buffer_size
            if severe_ae_rate > self.safety_threshold:
                # Extract event dicts for alerting
                recent_events = [e for _, e in self.ae_buffer]
