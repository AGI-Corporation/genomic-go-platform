...
class SafetyMonitoringSystem:
    ...
    async def process_adverse_event(self, event: Dict):
        ...
        # Parse timestamp once and store with event
        ...
        # O(1) Sliding window pruning: remove events older than 7 days
        ...
        # O(1) Check for safety signals using running aggregates
        if len(self.ae_buffer) > 10:  # Minimum for statistical power
            ...
            if severe_ae_rate > self.safety_threshold:
                # Convert buffer back to event list for alert (compatibility)
