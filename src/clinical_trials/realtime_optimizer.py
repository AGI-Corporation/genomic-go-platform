...
class SafetyMonitoringSystem:
    ...
    def __init__(self, trial_id: str, safety_threshold: float = 0.15):
        ...
        self.severe_count = 0  # Maintain a running count of severe AEs (severity >= 3)
        self.alert_history = []
    ...
    async def process_adverse_event(self, event: Dict):
        ...
        # Store as tuple (time, event) to avoid re-parsing during pruning
        self.ae_buffer.append((event_time, event))
        if event["severity"] >= 3:
            self.severe_count += 1

        # Prune events older than 7 days from the left (Sliding Window)
        ...
        if buffer_size > 10:  # Minimum for statistical power
            ...
            if severe_ae_rate > self.safety_threshold:
                # Convert back to list of dicts for the alert trigger if needed
