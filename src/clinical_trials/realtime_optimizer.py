...
class SafetyMonitoringSystem:
    ...
    def __init__(self, trial_id: str, safety_threshold: float = 0.15):
        ...
        self.ae_buffer = deque()  # Optimized: Use deque for O(1) pops from front
        self.severe_count = 0     # Optimized: Running counter for O(1) rate calculation
        self.alert_history = []
    ...
    async def process_adverse_event(self, event: Dict):
        ...
        # Optimized: Pre-parse timestamp and store in buffer to avoid redundant parsing
        ...
        # Optimized: O(1) amortized sliding window pruning using deque
        ...
        # Calculate rolling AE rate in O(1) using running severe_count
        if len(self.ae_buffer) > 10:  # Minimum for statistical power
            ...
            if severe_ae_rate > self.safety_threshold:
                # Only extract dictionaries if alert is actually triggered
