...
class RealTimePatientMatcher:
    ...
    async def create_patient_embedding(self, patient: PatientProfile) -> List[float]:
        """Create 768-dimensional embedding vector for patient profile using Mistral."""
        try:
            ...
            return await adapter.get_text_embedding(patient_text)
        except Exception as e:
            logger.warning(f"Mistral embedding failed, falling back to random: {e}")
