...
class SafetyMonitoringSystem:
    ...
    def __init__(self, trial_id: str, safety_threshold: float = 0.15):
        ...
        # Use deque for O(1) removals from the left (sliding window)
    ...
    async def process_adverse_event(self, event: Dict):
        """Process single AE and check for safety signals using an O(1) sliding window."""
        # Parse timestamp and ensure it's UTC-aware for robust comparison
        try:
            event_time = datetime.fromisoformat(event["timestamp"])
            if event_time.tzinfo is None:
                event_time = event_time.replace(tzinfo=timezone.utc)
        except (ValueError, KeyError) as e:
            logger.error(f"Invalid timestamp in event: {e}")
            return

        # Add new event to buffer and update severe count
        # Storing (event_time, event) avoids re-parsing timestamps during pruning
        self.ae_buffer.append((event_time, event))
        if event.get("severity", 0) >= 3:
            self.severe_count += 1

        # Prune events older than 7 days (O(1) amortized)
        # Assumes events arrive mostly in chronological order
        cutoff_time = datetime.now(timezone.utc) - timedelta(days=7)
        while self.ae_buffer and self.ae_buffer[0][0] < cutoff_time:
            expired_time, expired_event = self.ae_buffer.popleft()
            if expired_event.get("severity", 0) >= 3:
                self.severe_count -= 1
        ...
        if buffer_size > 10:  # Minimum for statistical power
            ...
            if severe_ae_rate > self.safety_threshold:
                # Pass list of events to trigger_safety_alert to maintain API compatibility
                await self.trigger_safety_alert(severe_ae_rate, [e[1] for e in self.ae_buffer])
