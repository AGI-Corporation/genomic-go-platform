...
class SafetyMonitoringSystem:
    ...
    def __init__(self, trial_id: str, safety_threshold: float = 0.15):
        ...
        self.ae_buffer = deque()  # Use deque for O(1) removals from front
        self.severe_count = 0  # Running counter for events with severity >= 3
        self.alert_history = []
    ...
    async def process_adverse_event(self, event: Dict):
        """Process single AE and check for safety signals using O(1) sliding window"""
        # Parse timestamp once and ensure it's UTC aware
        ...
        # Prune expired events (older than 7 days) from the front
        ...
        # Calculate rolling AE rate using pre-aggregated data
        total_recent = len(self.ae_buffer)
        if total_recent > 10:  # Minimum for statistical power
            severe_ae_rate = self.severe_count / total_recent

            if severe_ae_rate > self.safety_threshold:
                # Extract event dicts for compatibility with alert trigger
