...
class SafetyMonitoringSystem:
    ...
    async def process_adverse_event(self, event: Dict):
        ...
        # Store tuple (timestamp, event) to avoid re-parsing
        self.ae_buffer.append((event_time, event))
        if event["severity"] >= 3:
            self.severe_count += 1

        # O(1) amortized sliding window pruning (7-day window)
        ...
        # Check for safety signals using running counts
        ...
        if buffer_size > 10:  # Minimum for statistical power
            ...
            if severe_ae_rate > self.safety_threshold:
                # API compatibility: pass event dicts only
