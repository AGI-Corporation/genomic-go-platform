...
class SafetyMonitoringSystem:
    ...
    def __init__(self, trial_id: str, safety_threshold: float = 0.15):
        ...
        self.ae_buffer = deque()  # Optimized: Use deque for O(1) pruning
        self.severe_count = 0  # Optimized: Running counter for severe AEs
        self.alert_history = []
    ...
    async def process_adverse_event(self, event: Dict):
        ...
        # Add to buffer and update running counter
        self.ae_buffer.append((event_time, event))
        if event.get("severity", 0) >= 3:
            self.severe_count += 1

        # Optimized sliding window: prune events older than 7 days
        # This maintains O(1) amortized time complexity per event
        ...
        # Calculate rolling AE rate in O(1) using running counter
        ...
        if buffer_len > 10:  # Minimum for statistical power
            ...
            if severe_ae_rate > self.safety_threshold:
                ...
                recent_aes_dicts = [e[1] for e in self.ae_buffer]
                await self.trigger_safety_alert(severe_ae_rate, recent_aes_dicts)
