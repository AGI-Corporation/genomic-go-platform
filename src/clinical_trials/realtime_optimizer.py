...
class SafetyMonitoringSystem:
    ...
    def __init__(self, trial_id: str, safety_threshold: float = 0.15):
        ...
        self.ae_buffer = deque()  # Optimized O(1) removals from left
    ...
    async def process_adverse_event(self, event: Dict):
        """Process single AE and check for safety signals using optimized sliding window"""
        # Parse timestamp once and store it with the event to avoid redundant parsing
        ...
        # Update running severe count
        if event.get("severity", 0) >= 3:
            self.severe_count += 1

        # Prune old events (> 7 days) in O(1) amortized time
