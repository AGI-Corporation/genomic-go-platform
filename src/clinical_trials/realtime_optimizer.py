...
class SafetyMonitoringSystem:
    ...
    Implements a sliding window for efficient monitoring.
    """

    def __init__(self, trial_id: str, safety_threshold: float = 0.15):
        self.trial_id = trial_id
        self.safety_threshold = safety_threshold
        self.ae_buffer = collections.deque()  # O(1) append/popleft
    ...
    async def process_adverse_event(self, event: Dict):
        ...
        # Prune old events (older than 7 days) - O(1) amortized
        ...
        if buffer_size > 10:  # Minimum for statistical power
            ...
            if severe_ae_rate > self.safety_threshold:
                # Convert back to list only for the alert payload
