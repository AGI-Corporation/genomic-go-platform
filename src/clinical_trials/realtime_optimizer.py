...
class SafetyMonitoringSystem:
    ...
    async def process_adverse_event(self, event: Dict):
        ...
        Optimized to avoid re-scanning the entire buffer for each event.
        """
        # Parse event timestamp once
        event_time = datetime.fromisoformat(event["timestamp"])
        if event_time.tzinfo is None:
            event_time = event_time.replace(tzinfo=timezone.utc)

        # Add new event to buffer and update running severe count
        self.ae_buffer.append((event_time, event))
        if event.get("severity", 0) >= 3:
            self.severe_count += 1

        # Prune events older than 7 days from the front of the deque
        ...
        # Calculate rate in constant time
        if len(self.ae_buffer) > 10:
            severe_ae_rate = self.severe_count / len(self.ae_buffer)
            if severe_ae_rate > self.safety_threshold:
                # Extract events for alert (this part remains O(N) but only on alert)
