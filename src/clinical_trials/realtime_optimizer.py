...
class SafetyMonitoringSystem:
    ...
    def __init__(self, trial_id: str, safety_threshold: float = 0.15):
        ...
        self.ae_buffer = deque()  # Sliding window of recent AEs
        self.severe_count = 0  # Running count of severe AEs in current window
        self.alert_history = []
    ...
    async def process_adverse_event(self, event: Dict):
        ...
        Optimized with O(1) sliding window and running aggregates.
        """
        # Parse timestamp once
        event_time = datetime.fromisoformat(event["timestamp"])

        # Ensure UTC-awareness for robust comparison
        if event_time.tzinfo is None:
            event_time = event_time.replace(tzinfo=timezone.utc)

        # Store (timestamp, event) tuple to avoid re-parsing
        self.ae_buffer.append((event_time, event))

        # Update running aggregate for severe events (severity >= 3)
        if event.get("severity", 0) >= 3:
            self.severe_count += 1

        # Prune expired events (older than 7 days) from the sliding window
        ...
        # Calculate rolling AE rate using pre-computed counts
        if len(self.ae_buffer) > 10:  # Minimum for statistical power
            ...
            if severe_ae_rate > self.safety_threshold:
                # Pass list of current events to alert system
                current_events = [e for _, e in self.ae_buffer]
                await self.trigger_safety_alert(severe_ae_rate, current_events)
