...
class RealTimePatientMatcher:
    ...
    def check_eligibility(self, patient: PatientProfile, trial: Dict) -> bool:
        ...
        if required_markers and not required_markers.issubset(
            patient_variants
        ):
            return False
...
class SafetyMonitoringSystem:
    ...
    def __init__(self, trial_id: str, safety_threshold: float = 0.15):
        ...
        # Maintain running count of severe events for O(1) rate calculation
        self.severe_count = 0
        self.alert_history = []
    ...
    async def process_adverse_event(self, event: Dict):
        """Process single AE and check for safety signals using sliding window"""
        ...
        # Assuming events arrive mostly in order, this is highly efficient
        ...
        if len(self.ae_buffer) > 10:  # Minimum for statistical power
            ...
            if severe_ae_rate > self.safety_threshold:
                # Convert buffer back to list of events for alert compatibility
