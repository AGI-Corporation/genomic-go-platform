...
class AdaptiveTrialDesign:
    ...
    def check_stopping_rules(self) -> Tuple[bool, str]:
        ...
        for arm in self.arms:
            ...
            if p_success < 0.20:
                reason = f"Futility boundary crossed for {arm}: p={p_success:.3f}"
                return True, reason
...
class SafetyMonitoringSystem:
    ...
    async def process_adverse_event(self, event: Dict):
        ...
        Amortized O(1) complexity using deque and running counter.
        """
        ...
        while self.ae_buffer:
            ...
            if oldest_time < cutoff:
                expired_event = self.ae_buffer.popleft()
        ...
        if len(self.ae_buffer) > 10:  # Minimum for statistical power
            ...
            if severe_ae_rate > self.safety_threshold:
                # Convert deque to list for alert payload if needed
...
class EndpointPredictor:
    ...
    def engineer_features(
        ...
        """Transform raw trial data into model features."""
        return data
