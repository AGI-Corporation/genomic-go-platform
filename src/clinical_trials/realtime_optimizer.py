...
class SafetyMonitoringSystem:
    ...
    def __init__(self, trial_id: str, safety_threshold: float = 0.15):
        ...
        # Bolt Optimization: Use deque for O(1) sliding window pruning
        self.ae_buffer = deque()  # Stores (datetime, event_dict)
        self.severe_count = 0  # Running count of severe events (severity >= 3)
        self.alert_history = []
    ...
    async def process_adverse_event(self, event: Dict):
        ...
        # Bolt Optimization: O(1) amortized sliding window with running severe_count
        ...
        # Prune expired events (older than 7 days)
        ...
        if num_recent > 10:  # Minimum for statistical power
            ...
            if severe_ae_rate > self.safety_threshold:
                # Compatibility: trigger_safety_alert expects List[Dict]
