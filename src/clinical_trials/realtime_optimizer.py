...
class SafetyMonitoringSystem:
    ...
    def __init__(self, trial_id: str, safety_threshold: float = 0.15):
        ...
        self.ae_buffer = deque()  # Use deque for O(1) pruning from the left
        self.severe_count = 0  # Track severe events in the sliding window
        self.alert_history = []
    ...
    async def process_adverse_event(self, event: Dict):
        ...
        Optimized with O(1) sliding window and running aggregations.
        """
        # Parse timestamp once and ensure it's UTC-aware for consistent comparison
        ...
        # O(1) pruning of expired events (older than 7 days)
        # Using a sliding window approach to avoid O(N) re-scanning
        ...
        # Check safety threshold using running counts
        if len(self.ae_buffer) > 10:  # Minimum for statistical power
            ...
            if severe_ae_rate > self.safety_threshold:
                # Convert back to list of events for compatibility with alert trigger
