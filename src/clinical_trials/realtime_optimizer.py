...
class SafetyMonitoringSystem:
    ...
    def __init__(self, trial_id: str, safety_threshold: float = 0.15):
        ...
        self.ae_buffer = deque()  # Use deque for O(1) pruning from front
        self.severe_count = 0  # Running count for O(1) rate calculation
        self.alert_history = []
    ...
    async def process_adverse_event(self, event: Dict):
        """
        Process single AE and check for safety signals using a sliding window.
        Optimized for O(1) amortized complexity.
        """
        # Parse timestamp once and store in buffer
        ...
        # Prune events older than 7 days from the sliding window
        # Assumes events are mostly chronological
        cutoff_time = datetime.now(timezone.utc) - timedelta(days=7)
        while self.ae_buffer and self.ae_buffer[0][0] < cutoff_time:
            expired_time, expired_event = self.ae_buffer.popleft()
            if expired_event["severity"] >= 3:
                self.severe_count -= 1

        # Calculate rolling AE rate in O(1)
        if len(self.ae_buffer) > 10:  # Minimum for statistical power
            ...
            if severe_ae_rate > self.safety_threshold:
                # Pass only the event dictionaries to the alert system
