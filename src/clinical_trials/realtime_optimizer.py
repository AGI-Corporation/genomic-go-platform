...
class SafetyMonitoringSystem:
    ...
    def __init__(self, trial_id: str, safety_threshold: float = 0.15):
        ...
        # Use deque for O(1) removals from the front (sliding window)
    ...
    async def process_adverse_event(self, event: Dict):
        ...
        # Parse timestamp once and ensure it is UTC-aware
        ...
        # Sliding window: prune events older than 7 days
        # This ensures O(1) amortized complexity and prevents memory leaks
        ...
        # Calculate severe AE rate in O(1) using running counter
        buffer_size = len(self.ae_buffer)
        if buffer_size > 10:
            severe_ae_rate = self.severe_count / buffer_size
            if severe_ae_rate > self.safety_threshold:
                # Compatibility: convert to list of dicts only when alert triggered
