...
class SafetyMonitoringSystem:
    ...
    def __init__(self, trial_id: str, safety_threshold: float = 0.15):
        ...
        self.severe_count = 0  # Running count for severity >= 3
        self.alert_history = []
    ...
    async def process_adverse_event(self, event: Dict):
        ...
        # Parse timestamp once and store as UTC-aware
        event_time = datetime.fromisoformat(event["timestamp"])
        if event_time.tzinfo is None:
            event_time = event_time.replace(tzinfo=timezone.utc)

        # Add to buffer and update running aggregation
        self.ae_buffer.append((event_time, event))
        if event["severity"] >= 3:
            self.severe_count += 1

        # Sliding window: prune events older than 7 days from the front
        # Assumes events are mostly chronological (typical for real-time streams)
        window_start = datetime.now(timezone.utc) - timedelta(days=7)

        while self.ae_buffer and self.ae_buffer[0][0] < window_start:
            pruned_time, pruned_event = self.ae_buffer.popleft()
            if pruned_event["severity"] >= 3:
                self.severe_count -= 1

        # Calculate rate in O(1)
        if len(self.ae_buffer) > 10:  # Minimum for statistical power
            ...
            if severe_ae_rate > self.safety_threshold:
                # Convert back to list format for the alert trigger if needed
