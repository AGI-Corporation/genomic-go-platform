...
class SafetyMonitoringSystem:
    ...
    def __init__(self, trial_id: str, safety_threshold: float = 0.15):
        ...
        self.ae_buffer = deque()  # O(1) appends and pops
        self.severe_count = 0  # Running counter for severity >= 3
        self.alert_history = []
    ...
    async def process_adverse_event(self, event: Dict):
        ...
        Optimized sliding window implementation (O(1) amortized).
        """
        # Store event with parsed timestamp to avoid repeated parsing
        try:
        ...
        # we can stop as soon as we find a non-expired event.
