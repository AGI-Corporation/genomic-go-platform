...
class SafetyMonitoringSystem:
    ...
    def __init__(self, trial_id: str, safety_threshold: float = 0.15):
        ...
        self.ae_buffer = collections.deque()  # Using deque for O(1) removals
        self.severe_count = 0  # Running count of severe events in buffer
        self.alert_history = []
    ...
    async def process_adverse_event(self, event: Dict):
        ...
        # 1. Add new event to buffer and update running count
        self.ae_buffer.append((event_time, event))
        if event["severity"] >= 3:
            self.severe_count += 1

        # 2. Prune old events (older than 7 days) - Sliding Window O(1) amortized
        ...
        if buffer_size > 10:  # Minimum for statistical power
            ...
            if severe_ae_rate > self.safety_threshold:
                # Convert buffer back to list of events for the alert
