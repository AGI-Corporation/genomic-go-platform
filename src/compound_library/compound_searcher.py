...
class CompoundSearcher:
    ...
    def _build_filter(self, filters: Dict) -> models.Filter:
        ...
        if "molecular_weight_range" in filters:
            ...
            conditions.append(
                models.FieldCondition(
                    key="molecular_weight", range=models.Range(gte=min_mw, lte=max_mw)
                )
            )
        ...
        if "mechanism" in filters:
            conditions.append(
                models.FieldCondition(
                    key="mechanism", match=models.MatchValue(value=filters["mechanism"])
                )
            )
    ...
    def find_similar_compounds(self, compound_id: str, limit: int = 10) -> List[Dict]:
        ...
        reference = self.qdrant_client.retrieve(
            collection_name=self.collection_name, ids=[compound_id], with_vectors=True
        )
